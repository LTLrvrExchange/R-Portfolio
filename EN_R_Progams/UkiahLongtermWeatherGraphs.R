...
getwd()
setwd("G:/My Drive/01RVR_Exchange2016/02EnvirExchNetwkGrant/ENGrantDueProducts/EN_R_DataSources")
...
prep2021 <- mutate(prep, date = as.numeric(strftime(prep2021$DATE, format = "%d")))
prep2021 <- mutate(prep, Month = as.character(strftime(prep2021$DATE, format = "%B")))
prep2021 <- mutate(prep, month_number = as.numeric(strftime(prep2021$DATE, format = "%m")))
prep2021 <- mutate(prep, month_year = as.yearmon (strftime(prep2021$DATE, format = "%Y/%m")))
prep2021 <- mutate(prep, year = as.numeric(strftime(prep2021$DATE, format = "%Y")))
...
combined_data <- left_join(prepDailyAverages, prep2021, by = "DOY") %>%
                 rename("Average Temp - 2021" = "Average",
                "Mean Daily: 1893-2020" = "Mean") %>%

             pivot_longer(cols = c("Average Temp - 2021", "Mean Daily: 1893-2020"),
             names_to = "Parameter",
             values_to = "Value")

#Create a Continuous Line Graph with Ribbon

wthr <- ggplot(data = combined_data, aes(DOY,Value, color = Parameter))+
    ...
    # scale_x_date(breaks = seq(min(prepDailyAverages$year), max(prepDailyAverages$year))+
